...
const LpDetailPage = () => {
  ...
  const likeMutation = useMutation({
    mutationFn: ({ isLiked }: { isLiked: boolean }) =>
      isLiked ? unlikeLp(parsedLpId) : likeLp(parsedLpId),
    onMutate: async ({ isLiked }) => {
      await Promise.all([
        queryClient.cancelQueries({ queryKey: [QUERY_KEY.lp, parsedLpId] }),
        queryClient.cancelQueries({ queryKey: [QUERY_KEY.lps] }),
      ]);
      ...
      const previousLpLists =
        queryClient.getQueriesData<InfiniteData<ResponseLpListDto>>({
          queryKey: [QUERY_KEY.lps],
        });

      if (currentUserId === null) {
        return { previousLpDetail, previousLpLists };
      }
      ...
      const getOptimisticLikes = (
        likes: ResponseLpDetailDto["data"]["likes"],
      ) => {
        if (isLiked) {
          return likes.filter((like) => like.userId !== currentUserId);
        }

        const alreadyLiked = likes.some(
          (like) => like.userId === currentUserId,
        );
        return alreadyLiked ? likes : [...likes, optimisticLike];
      };

      queryClient.setQueryData<ResponseLpDetailDto>(
        [QUERY_KEY.lp, parsedLpId],
        (oldLpDetail) => {
          if (!oldLpDetail) return oldLpDetail;

          return {
            ...oldLpDetail,
            data: {
              ...oldLpDetail.data,
              likes: getOptimisticLikes(oldLpDetail.data.likes),
            },
          };
        },
      );

      queryClient.setQueriesData<InfiniteData<ResponseLpListDto>>(
        { queryKey: [QUERY_KEY.lps] },
        (oldLpLists) => {
          if (!oldLpLists) return oldLpLists;

          return {
            ...oldLpLists,
            pages: oldLpLists.pages.map((page) => ({
              ...page,
              data: {
                ...page.data,
                data: page.data.data.map((lp) =>
                  lp.id === parsedLpId
                    ? { ...lp, likes: getOptimisticLikes(lp.likes) }
                    : lp,
                ),
              },
            })),
    ...
    onError: (_error, _variables, context) => {
      ...
      context?.previousLpLists?.forEach(([queryKey, previousLpList]) => {
        queryClient.setQueryData(queryKey, previousLpList);
      });
    },
    onSettled: async () => {
      await Promise.all([
      ...
      <div className="mx-auto max-w-5xl rounded-lg bg-[#2a2d34] px-6 py-8 shadow-2xl sm:px-10 lg:px-16">
        ...
        <div className="mt-10 flex items-center justify-center gap-3 text-2xl font-bold">
          <button
            ...
            className={`transition-colors hover:text-[#ff1493] disabled:cursor-not-allowed disabled:opacity-50 ${
              isLiked ? "text-[#ff5d8f]" : "text-white"
            }`}
          >

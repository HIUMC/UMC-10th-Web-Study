...
export const getLpList = async (order: "asc" | "desc", cursor?: number, search?: string) => {
  const { data } = await axiosInstance.get("/v1/lps", {
    params: { cursor: cursor ?? 0, limit: 10, order, ...(search ? { search } : {}) },
  });
  return data.data;
};
...
export const updateLp = async (
  lpId: number,
  payload: Partial<CreateLpPayload>
) => {

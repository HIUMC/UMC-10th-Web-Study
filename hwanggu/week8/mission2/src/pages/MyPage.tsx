...
const MyPage = () => {
  ...
  const updateMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      // 캐시를 즉시 새 값으로 업데이트 → 마이페이지 닉네임 즉시 반영
      queryClient.setQueryData(["myInfo"], (old: any) => ({
        ...old,
        data: {
          ...old?.data,
          name: editName || old?.data?.name,
          bio: editBio || old?.data?.bio,
          avatar: editAvatar || old?.data?.avatar,
        },
      }));

      // Navbar의 nickname도 즉시 업데이트
      if (editName) {
    ...
    onError: (_err, _vars, context) => {
      ...
      if (prev?.data?.name) {
        localStorage.setItem("nickname", prev.data.name);
      }
      alert("프로필 수정에 실패했습니다.");
    },

    // 3) 성공/실패 상관없이 서버 데이터로 최종 동기화
    onSettled: () => {
  ...
  if (isError) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-black text-white gap-4">
        ...
        <button onClick={() => navigate("/login")} className="px-6 py-2 rounded-lg bg-[#FF2E7E] text-white font-bold">
          로그인하러 가기
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen bg-black text-white gap-6 p-8">
      ...
      <div className="bg-[#111] p-8 rounded-2xl flex flex-col items-center gap-4 w-full max-w-sm">
        <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#FF2E7E", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, fontWeight: "bold" }}>
          {user?.avatar
        ...
        <button onClick={openEdit} style={{ padding: "8px 24px", borderRadius: 8, background: "#222", border: "1px solid #444", color: "#fff", cursor: "pointer", fontSize: 14 }}>
          ⚙️ 프로필 수정
        </button>
      </div>

      <button onClick={() => logoutMutation.mutate()} disabled={logoutMutation.isPending}
        style={{ padding: "10px 32px", borderRadius: 8, background: "#222", border: "1px solid #444", color: "#fff", cursor: "pointer", fontSize: 14 }}>
          ...
          <div style={{ background: "#1a1a1a", borderRadius: 16, padding: "2rem", width: "100%", maxWidth: 400, display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              <div onClick={() => fileRef.current?.click()}
                ...
                {editAvatar ? <img src={editAvatar} alt="avatar" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "📷"}
              </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#1a1a1a", borderRadius: 16, padding: "2rem", width: "100%", maxWidth: 340, textAlign: "center", display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { setShowDeleteConfirm(false); deleteMutation.mutate(); }} disabled={deleteMutation.isPending}

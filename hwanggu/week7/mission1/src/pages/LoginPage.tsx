...
const LoginPage = () => {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      localStorage.setItem("nickname", myInfo.data.name);

      const state = location.state as { from?: { pathname?: string } } | null;
      const from = state?.from?.pathname || "/lps";
    ...
    <div className="flex justify-center items-center h-screen bg-black">
      <form onSubmit={handleSubmit(onSubmit)} className="w-87.5 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          />
          {emailError && (
            <span className="text-[#FF2E7E] text-xs ml-1">{errors.email}</span>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          />
          {passwordError && (
            <span className="text-[#FF2E7E] text-xs ml-1">{errors.password}</span>
          )}
        </div>
        ...
        <p className="text-center text-[#666] text-sm">
          ...
          <span
            ...
            className="text-[#FF2E7E] cursor-pointer hover:underline"
          >
            회원가입
          </span>
        </p>

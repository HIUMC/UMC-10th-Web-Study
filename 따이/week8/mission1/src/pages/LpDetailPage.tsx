...
import { addLike, deleteLike, deleteLp } from "../apis/lp";
...
import type { Lp, ResponseGetLpDto } from "../types/lp";
...
const LpDetailContent = ({
  ...
  const { mutate: mutateLike } = useMutation({
    mutationFn: () => (isLiked ? deleteLike(id) : addLike(id)),
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: lpQueryKey(id) });
      const previous = queryClient.getQueryData<ResponseGetLpDto>(lpQueryKey(id));
      queryClient.setQueryData<ResponseGetLpDto>(lpQueryKey(id), (old) => {
        if (!old) return old;
        const currentLikes = old.data.likes ?? [];
        const newLikes = isLiked
          ? currentLikes.filter((l) => l.userId !== myUserId)
          : [...currentLikes, { id: -1, userId: myUserId ?? 0, lpId: id }];
        return { ...old, data: { ...old.data, likes: newLikes } };
      });
      return { previous };
    },
    onError: (_err, _vars, context) => {
      queryClient.setQueryData(lpQueryKey(id), context?.previous);
    },
    onSettled: () => {

    ...
    const fetchData = async (currentRetry = 0) => {
      ...
      if (cachedItem) {
        try {
          ...
          // 캐시가 만료된 경우
          setData(cachedData.data); // 만료된 데이터라도 일단 보여주기
          console.log("만료된 캐시 데이터 사용", url);
        } catch {
          localStorage.removeItem(storageKey); // 파싱 실패 시 캐시 삭제
      ...
      try {
        ...
        if (currentRetry < MAX_RETRIES) {
          const retryDelay = INITIAL_RETRY_DELAY * Math.pow(2, currentRetry); // 지수적 백오프
          console.log(
          ...
          setIsError(true);
          setIsPending(false);
          console.log("최대 재시도 횟수 초과, 에러 발생:", url);
          return;
        }

...
const MovieDetailPage = () => {
  ...
  // 출연진(크레딧) 정보 요청 — 상세 정보와 독립적으로 병렬 요청됨
  const {
    data: credits,
  ...
  // 두 요청 중 하나라도 실패하면 에러 처리
  const isError = isMovieError || isCreditsError;

  // 출연진 최대 20명까지만 표시, credits가 아직 없으면 빈 배열
    ...
    return (
      <div className="flex items-center justify-center h-dvh">
        <span className="text-red-500 text-2xl">
          영화 정보를 불러오는 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="relative w-full" style={{ minHeight: 420 }}>
        {/* backdrop_path가 있을 때만 배경 이미지 렌더링 */}
        {movie.backdrop_path && (
